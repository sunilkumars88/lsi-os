...
async def run_agent_internal(body: AgentRunIn) -> AgentRunOut:
    """Unauthenticated Nest-friendly agent invoke (demo / OpenAI via llm_router)."""
    agent_type = body.agentType or "researcher"
    ...
    system = PROMPT_LIBRARY.get(agent_type, PROMPT_LIBRARY["copilot"])
    llm = await llm_router.complete(
        [{"role": "user", "content": f"Run {agent_type} agent.\nInput: {query}"}],
        system=system,
    )
    return AgentRunOut(
...
async def agents_run(body: AgentRunIn) -> AgentRunOut:
    """Internal route for Nest — no JWT required."""
